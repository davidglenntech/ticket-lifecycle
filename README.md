...
_As an Agent, we'll observe and configure the information of this ticket._

_Having the entire mobile online banking down is something that could have a major impact on the company, resulting in losing money. The severity of this should be higher and should be assigned to the departments/teams that can be responsible to resolve this issue ASAP!_
- Set Priority from Normal to a higher level (this example uses **Emergency**).
- Assign to a higher-tier department (this example uses **System Administrators**).
- Assign a specific person(s) the responsibility to manage this ticket (this example uses the current user, **Jane Doe**).
...
- Keep the Ticket Status to "Open (current)", assuming the issues aren't resolved.
- Click "Post Reply".
<p align=center>
...
- Under Post Reply, type in a random message stating a final update on the matter.
...
_You can find it under the "Closed" tab, where you can see how many were closed at a certain time frame._
<p align=center>
<img src="https://i.imgur.com/xw9gHmX.jpg" height="80%" width="80%" alt="Disk Sanitization Steps"/>
</p>

- Continue to go through the rest of the remaining tickets and use the best judgment on their Priorty, assignment to departments and teams, etc.
<p align=center>
